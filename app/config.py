...
@dataclass
class PingTask(
    Schedulable, SupportsMerge["PingTask"], ValidationMixin, NamedMixin, FromDictMixin
):
    target_host: str
    count: int = 5
...
@dataclass
class SpeedtestTask(
    Schedulable, SupportsMerge["SpeedtestTask"], ValidationMixin, NamedMixin
):
    additional_flags: str = ""
    id: str = "speedtest"
...
class SpeedtestConfig(SupportsMerge["SpeedtestConfig"], ValidationMixin):
    ...
    def from_dict(cls, data: dict) -> Self:
        tasks = [
            SpeedtestTask.from_dict(task_data) for task_data in data.get("tasks", [])
        ]
    ...
    def from_env(cls) -> Self | None:
        ...
        accept_gdpr = (
            True
            if SPEEDTEST_GDPR_ACCEPT_ENV in os.environ
            and os.environ.get(SPEEDTEST_GDPR_ACCEPT_ENV, "").lower() in YES_VALUES
            else False
        )
        accept_license = (
            True
            if SPEEDTEST_LIC_ACCEPT_ENV in os.environ
            and os.environ.get(SPEEDTEST_LIC_ACCEPT_ENV, "").lower() in YES_VALUES
            else False
        )
        if task is not None:
    ...
    def from_cli_args(cls, args) -> Self | None:
        ...
        accept_gdpr = (
            args.accept_speedtest_gdpr is not None and args.accept_speedtest_gdpr
        )
        accept_license = (
            args.accept_speedtest_license is not None and args.accept_speedtest_license
        )
        if task is not None:
            return cls(cmd, [task], accept_gdpr, accept_license)
        return cls(cmd, [], accept_gdpr, accept_license)
...
class InfluxConfig(SupportsMerge["InfluxConfig"], ValidationMixin):
    ...
    def from_env(cls) -> Self | None:
        ...
        token = (
            ...
            if token_file
            else os.environ.get("INFLUXDB_TOKEN")
        )
    ...
    def from_cli_args(cls, args) -> Self | None:
        ...
        retry_interval = (
            args.retry_interval if args.retry_interval is not None else 5000
        )
        max_retry_time = (
            args.max_retry_time
...
class Config(ValidationMixin):
    ...
    def load(self, args):
        ...
        self.grace_time = (
            args.get("grace_time", None) or int(os_grace) if os_grace else None
        )
